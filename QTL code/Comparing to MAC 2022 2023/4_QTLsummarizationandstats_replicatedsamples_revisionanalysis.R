#Code by Randi Avery for --REVISIONS-- of
...
#revision analysis to only consider QTLs present in MAC 2022, 2023 and absent in Avery et al 2024
#same script as other comparison script, but just edited to go the other direction. See "#revision" wherever a change was made
...
#we both started with 59 QTLs!


#revision
#only want QTLs in MAC data that are present in both replicates to plot total QTLs
qtls2_mac = qtls_mac[qtls_mac$number_of_reps == 2,] #31 QTLs found in both replicates of MAC (was 39 in RRA)
...
#revision
qtls2_mac$rep = paste0(qtls2_mac$reporter, '_MAC')
#revision
qtls_rra$rep = paste0(qtls_rra$reporter, '_RRA') #59 QTLs
...
rra_mac_combos$first1 = rra_mac_combos$first
rra_mac_combos = rra_mac_combos[c("second", "first1")]
colnames(rra_mac_combos) = c("first", "second") #revision - now the comparison will go in the other direction

#recombine my and Mahlon's qtls
qtls_mac_rra = rbind(qtls2_mac, qtls_rra) #revision 90
...
overlapsMR_merge = overlapsMR[[2]] #68 overlapping pairs #revision 66 overlapping pairs
...
#revision: didn't save data tables for reloading since this is a short script
#save QTL info table
...
overlapsMR_merge_filtered = overlapsMR_merge[overlapsMR_merge$total_reps != 1,] #50 overlapping pairs. Has P/A with 2 reps in one sample and 0 in the other - either in RRA or MAC! #revision 50 pairs
...
overlapsMR_merge_filtered = overlapsMR_merge_filtered[overlapsMR_merge_filtered$remrow != '2TRUE',  ] #still 50 #revision still 50
overlapsMR_merge_filtered = overlapsMR_merge_filtered[overlapsMR_merge_filtered$remrow != '2FALSE',  ] #still 50 #revision still 50
...
##revision
#DON'T KEEP QTLS THAT WERE IN 2 REPS OF mine AND NONE OF mahlon's - because we are using MAC's QTLs as the focal list for revisions
#because in the combolist I always listed MAC's sample first, the only case this will be true is P/A and MAC is in the rep1 column
overlapsMR_merge_filtered2 = overlapsMR_merge_filtered[overlapsMR_merge_filtered$rep1 != 'Asn_RRA',] #47 #revision 49
overlapsMR_merge_filtered2 = overlapsMR_merge_filtered2[overlapsMR_merge_filtered2$rep1 != 'Phe_RRA',] #44 #revision 42
overlapsMR_merge_filtered2 = overlapsMR_merge_filtered2[overlapsMR_merge_filtered2$rep1 != 'Rpn4_RRA',] #40 #revision 37
overlapsMR_merge_filtered2 = overlapsMR_merge_filtered2[overlapsMR_merge_filtered2$rep1 != 'Thr_RRA',] #39 #revision 31 #should be the number of focal QTLs we start out with
...
#1/3 DF#
mr_notabsent = overlapsMR_merge_filtered2[overlapsMR_merge_filtered2$true_means_same_dir != 'Absent',] #30 #revision 24
mr_absent = overlapsMR_merge_filtered2[overlapsMR_merge_filtered2$true_means_same_dir == 'Absent',] #9 #revision 7
#revision ALL 9 MAC THR QTLS WERE FOUND IN RRA!!


#for sign change or no GxE ('notabsent') (will result in all 0's for P/A) #repurposed code
...
#now do P/A
#ONLY EVER PRESENT IN MY mac AND ABSENT IN my DATA (already analyzed the other way around) #revision

#run function
mr_PA = gxestats_fun(mr_absent, 'rep1')
#revision - missing Thr row because none PA
#add row for Thr
mr_PA= rbind(mr_PA, c("Thr_MAC", 0,0,0,0,0,0,0))
...
#revision
mr_PA$'Present in MAC; Absent in RRA' = mr_PA$`Presence/Absense GxE (2Absent)`
#revision
keeps2mr = c('Reporter', 'Present in MAC; Absent in RRA')
mr_notabsentstat2 = mr_PA[keeps2mr]

mr_stats_done = merge(mr_notabsentstat1,mr_notabsentstat2, by = 'Reporter')
#revision - not sure where the final column turned to chr
mr_stats_done$`Present in MAC; Absent in RRA` = as.integer(mr_stats_done$`Present in MAC; Absent in RRA`)
mr_stats_done$`Total Overlapping Pairs` = mr_stats_done$`Sign change GxE (4FALSE)` + mr_stats_done$`Sign change GxE (3FALSE)` + mr_stats_done$`Overlapping but NO GxE (4TRUE)` + mr_stats_done$`Overlapping but NO GxE (3TRUE)` + mr_stats_done$`Present in MAC; Absent in RRA` #revision
mr_stats_done$`Missing %` = (mr_stats_done$`Present in MAC; Absent in RRA` / mr_stats_done$`Total Overlapping Pairs`) *100 #revision
...
write.table(mr_stats_done,
            file = "MAC_RRAoverlapping_stats25.03.11.txt",
            quote = FALSE,
            row.names = FALSE,
            col.names = TRUE,
            sep = '\t')
...
#revision
#but prob only want to look at stats for just MAC QTLs, cuz I only have mac QTLs for P/A
#revision - called "rra_annotatedQTLs" but in this script, the annotated QTLs are from Mahlon's
rra_annotatedQTLs = annotatedQTLs[annotatedQTLs$rep == 'Asn_MAC',]
rra_annotatedQTLs = rbind(rra_annotatedQTLs, annotatedQTLs[annotatedQTLs$rep == 'Phe_MAC',])
rra_annotatedQTLs = rbind(rra_annotatedQTLs, annotatedQTLs[annotatedQTLs$rep == 'Thr_MAC',])
rra_annotatedQTLs = rbind(rra_annotatedQTLs, annotatedQTLs[annotatedQTLs$rep == 'Rpn4_MAC',])
#revision - 31 QTLs - number of QTLs found in these 4 samples in both replicates (of MAC)


write.table(rra_annotatedQTLs,
            file = "MAC_QTLs_replicated25.03.11.txt",
            quote = FALSE,
            row.names = FALSE,
            col.names = TRUE,
            sep = '\t')
...
#reload
rra_annotatedQTLs = read.table(file = "MAC_QTLs_replicated25.03.11.txt", header = TRUE, sep = '\t')
...
#Want to see where LOD, dAF lay for the QTLs that didn't replicate and those that did
...
#saved as 3x3.25 previously - for revisions just saved a screenshot
